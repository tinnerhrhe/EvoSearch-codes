...
from flux_evol_pipe import EvolFluxPipeline
...
def get_args():
    ...
    parser.add_argument('--model_name', type=str, default='black-forest-labs/FLUX.1-dev', help='pre-trained model name')
    ...
    parser.add_argument('--elite_size', type=int, default=3, help='Number of elites to keep across each generation')
...
if __name__ == '__main__':
    ...
    pipe = EvolFluxPipeline.from_pretrained(
        args.model_name,
    ...
    for idx, row in df.iterrows():
        prompt = row["Prompts"]
        best_reward = -100
        shape = (args.population_size_schedule[0], pipe.transformer.config.in_channels // 4, 2 * (int(args.image_size) // (pipe.vae_scale_factor * 2)), 2 * (int(args.image_size) // (pipe.vae_scale_factor * 2)))
        init_latents = torch.randn(shape, generator=generator, dtype=dtype).to(device)
        init_latents = pipe._pack_latents(init_latents, args.population_size_schedule[0], pipe.transformer.config.in_channels // 4, 2 * (int(args.image_size) // (pipe.vae_scale_factor * 2)), 2 * (int(args.image_size) // (pipe.vae_scale_factor * 2)))
